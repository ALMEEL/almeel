...
const kg=n=>(Number(n)||0).toFixed(1)+' kg';
...
window.deleteBody=id=>{if(confirm('Delete check-in?')){set('almeel_body',get('almeel_body',[]).filter(x=>String(x.id)!==String(id)));renderAll()}};
window.editBody=id=>{let a=get('almeel_body',[]),x=a.find(v=>String(v.id)===String(id));if(!x)return;let w=prompt('Weight kg:',x.weight);if(w===null)return;let waist=prompt('Waist cm:',x.waist||'');if(waist===null)return;let notes=prompt('Notes:',x.notes||'');if(notes===null)return;x.weight=Number(w);x.waist=waist?Number(waist):'';x.notes=notes;set('almeel_body',a);renderAll()};
...
function renderBody(){
  let a=get('almeel_body',[]).filter(x=>x.weight).sort((x,y)=>x.date.localeCompare(y.date));
  let latest=a[a.length-1], first=a[0], prev=a[a.length-2];
  $('#bodyEntries').textContent=a.length;
  $('#bodyStart').textContent=first?kg(first.weight):'—';
  $('#bodyNow').textContent=latest?kg(latest.weight):'—';
  $('#bodyTotal').textContent=(first&&latest)?((latest.weight-first.weight)>=0?'+':'')+(latest.weight-first.weight).toFixed(1)+' kg':'—';
  $('#currentWeight').textContent=latest?kg(latest.weight):'—';
  $('#weightChange').textContent=(latest&&prev)?((latest.weight-prev.weight)>=0?'+':'')+(latest.weight-prev.weight).toFixed(1)+' kg':'—';
  $('#bodyList').innerHTML=a.slice().reverse().map(x=>`<div class="entry"><div class="row"><b>⚖️ ${kg(x.weight)}</b><span class="badge">${x.date}</span></div><p>${x.waist?'Waist: '+x.waist+' cm · ':''}${x.chest?'Chest: '+x.chest+' cm · ':''}${x.arm?'Arm: '+x.arm+' cm':''}</p><p>${x.notes||''}</p><div class="actions"><button class="mini" onclick="editBody('${x.id}')">✏️ Edit</button><button class="mini danger" onclick="deleteBody('${x.id}')">🗑 Delete</button></div></div>`).join('')||'<p>No body check-ins yet.</p>';
  renderWeightChart(a);
}
function renderWeightChart(data){
  const svg=$('#weightChart'); if(!svg) return;
  if(data.length<2){svg.innerHTML='<text x="350" y="150" text-anchor="middle" class="chartText">Add at least 2 weekly weights to show a chart</text>';$('#chartMin').textContent='Min —';$('#chartMax').textContent='Max —';return}
  const W=700,H=300,p=34, weights=data.map(x=>Number(x.weight)), min=Math.min(...weights), max=Math.max(...weights), span=(max-min)||1;
  const pts=data.map((d,i)=>{let x=p+(i/(data.length-1))*(W-2*p);let y=H-p-((d.weight-min)/span)*(H-2*p);return {x,y,w:d.weight,date:d.date}});
  const path=pts.map((pt,i)=>(i?'L':'M')+pt.x.toFixed(1)+' '+pt.y.toFixed(1)).join(' ');
  const area=`M ${pts[0].x} ${H-p} L `+pts.map(pt=>pt.x+' '+pt.y).join(' L ')+` L ${pts[pts.length-1].x} ${H-p} Z`;
  svg.innerHTML=`<line x1="${p}" y1="${p}" x2="${p}" y2="${H-p}" class="chartGrid"/><line x1="${p}" y1="${H-p}" x2="${W-p}" y2="${H-p}" class="chartGrid"/><path d="${area}" class="chartArea"/><path d="${path}" class="chartLine"/>`+
  pts.map((pt,i)=>`<circle cx="${pt.x}" cy="${pt.y}" r="6" class="chartDot"><title>${pt.date}: ${pt.w} kg</title></circle>`).join('')+
  pts.map((pt,i)=> i===0||i===pts.length-1?`<text x="${pt.x}" y="${H-8}" text-anchor="middle" class="chartText">${pt.date.slice(5)}</text>`:'').join('');
  $('#chartMin').textContent='Min '+kg(min); $('#chartMax').textContent='Max '+kg(max);
}
...
function renderAll(){renderEvents();renderActivities();renderExpenses();renderJournal();renderBody();renderExercises();renderToday();renderStats();runTimer()}
...
$('#saveBody').onclick=()=>{let weight=Number($('#bodyWeight').value);if(!weight){alert('اكتب الوزن');return}let a=get('almeel_body',[]);a.push({id:Date.now(),date:$('#bodyDate').value||today(),weight,waist:$('#bodyWaist').value?Number($('#bodyWaist').value):'',chest:$('#bodyChest').value?Number($('#bodyChest').value):'',arm:$('#bodyArm').value?Number($('#bodyArm').value):'',notes:$('#bodyNotes').value});set('almeel_body',a);['bodyWeight','bodyWaist','bodyChest','bodyArm','bodyNotes'].forEach(id=>$('#'+id).value='');renderAll();alert('Body check-in saved ✅')};
...
$('#eventDate').value=today(); $('#bodyDate').value=today();
